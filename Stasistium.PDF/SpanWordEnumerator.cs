...
namespace Stasistium.PDF
{
    ...
    public ref struct SpanWordEnumerator
    {
        ...
        private static readonly string WHITESPACE_CHARACTERS = string.Join("", System.Linq.Enumerable.Range(0, 255).Select(x => (char)x).Where(char.IsWhiteSpace));
        ...
        public bool MoveNext()
        {
            ...
            int stride = 0;
            while (endOfOldString + stride < buffer.Length && splitCharacters == null ? char.IsWhiteSpace(buffer[endOfOldString + stride]) : (splitCharacters ?? WHITESPACE_CHARACTERS).Contains(buffer[endOfOldString + stride])) {
                stride++;
            }
            int beginningOfNewString = endOfOldString + stride;

            int endOfString = buffer[beginningOfNewString..].IndexOfAny(splitCharacters ?? WHITESPACE_CHARACTERS);
            if (endOfString == -1) {
                current = ^0..^0;
                return false;
            }
        ...
        public bool MovePrevious()
        {
            ...
            int stride = 0;
            while (startOfOldString - stride > 0 && splitCharacters == null ? char.IsWhiteSpace(buffer[startOfOldString - stride]) : (splitCharacters ?? WHITESPACE_CHARACTERS).Contains(buffer[startOfOldString - stride])) {
                stride++;
            }
            int endOfNewString = startOfOldString - stride;

            int beginningOfString = buffer[..endOfNewString].LastIndexOfAny(splitCharacters ?? WHITESPACE_CHARACTERS);
